...
留有余量、最大间隔、最小化结构风险的思想，先定义（找出）离超平面最近的样本点（该样本点就叫支持向量）距离（一般都是规划化最近样本点到超平面距离为1）所以同一类的其余所有样本点到超平面的距离都应该大于等于最近样本点到超平面的距离，之后转换为优化（最大化）两类的支持向量的距离，也就可以等价于最小化权矢量的二范数（模长）。关于为什么等价最小化权矢量二范数，这个我不想写了，有太多要写了，前面还有好多坑没写呢，说个大概吧，就是第二章中线性分类器的判别函数的鉴别里面（我贴了一张图，图里面第二条性质，证明很简单的）。自带稀疏性，只有极少数位于边界上的点参与计算。关于出现的证明其实也挺好做，出现有约束的极值无非就是拉格朗日乘子法，但是支持向量机还有一个非常简单的强队偶方法可以解决极值推导问题。
...
无监督分类，不需要使用训练样本进行学习和训练。

如何度量相似性（距离远近），欧氏距离和明氏距离（对受特征的量纲波动干扰较大），马氏距离（对量纲不敏感，自带归一化操作）等。准则函数（与Fisher线性判别器一样，最大类间离差，最小类内离差）。当然毕竟是理论分析，工程实践落地都是理论的近似（因为实现需要考虑的因素太多，时间、空间复杂度）。寻找解优化时，当然也要考虑时间、空间复杂度，所以一般不会采取暴力遍历枚举、直接极值，一般都是采取启发式算法（利用经验、直觉、常识等找到一个较优解，不一定能找到全局最优解，一般都是找到的局部最优解，落入较近似的极值）寻找解。
