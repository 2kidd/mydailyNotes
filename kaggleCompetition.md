...
直接干到16GB（一张单卡16G）了，直接显存爆炸，不现实，而且这里还只是算的静态加载参数进来，在训练阶段还会有许多中间变量存储（梯度反向传播时梯度值、优化器状态、中间激活值），输入数据的显存占用还没加上，会需要占用更多显存，并且这里还是采用的精度不是很高的FP16类型数据存储。所以必须采取某些技术冻结大部分参数，微调部分参数。
