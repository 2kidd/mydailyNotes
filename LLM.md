...
这一章其实就是一些名词，随便看看得了，没啥好记的，能简单区分，知道有这个东西就行了。

## Chapter 2. Transformer

自回归的“生成模型”只关注前序词元（GPT系列，仅解码式），而非自回归完形填空式可以关注两个方向（BERT）。其中有一篇文章“A Survey of Transformers”介绍了Transformer的改进方案的主要方向，以及在计算机视觉领域使用的Transformer架构，参考文章“A Survey on Vision Transformer”和“Transformer in Vision: A Survey”。Transformer的一个优点是可以并行处理tokens（词元）。

### 注意力机制

多头注意力、稀疏注意力、分组查询注意力（共享键矩阵和值矩阵，多查询注意力的改进把版本）、多查询注意力（共享键矩阵和值矩阵）、Flash Attention（优化GPU操作方式加速注意力的计算）

#### self-attention：

包含两个主要步骤1.相关性评分，2.信息组合
